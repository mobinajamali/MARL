...
class Replay:
    ...
    memory transition for each actor
    '''
    def __init__(self, mem_size, actor_dim, critic_dim, n_actions, n_agents, batch_size):
        ...
        self.state_mem = np.zeros((self.mem_size, self.critic_dim))
        self.new_state_mem = np.zeros((self.mem_size, self.critic_dim))
    ...
    def initialize_actor_mem(self):
        ...
        for i in range(self.n_agents):
            self.actor_state_mem.append(np.zeros((self.mem_size, self.actor_dim[i])))
            self.actor_new_state_mem.append(np.zeros((self.mem_size, self.actor_dim[i])))
